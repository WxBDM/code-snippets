...
class DoublyLinkedStructures(object):
    ...
    def __getitem__(self, position):
        ...
        # create the pointer
        pointer = self._create_pointer(rear = False if position >= 0 else True)
...
class Stack(DoublyLinkedStructures):
...
class Queue(DoublyLinkedStructures):
    '''Queue data structure. Supports double ended queue (deque) behavior
        on a time basis rather than priority basis.
...
class Bag(DoublyLinkedStructures):
    ...
    def remove(self, element):
        '''Removes the first instance of the specified element, if it exists.
        Returns the number if removed, returns false if it wasn't removed.'''
...
class Set(DoublyLinkedStructures):
